...
Includes read and write serializers. `OrderCreateSerializer` accepts an
`offer_detail_id` and constructs an `Order` copying the relevant fields
from the chosen `OfferDetail`.
"""
...
class OrderCreateSerializer(serializers.Serializer):
    """Create an Order from an `offer_detail_id`.

    The serializer validates that the provided OfferDetail exists and
    then creates an Order copying the title, price, features and other
    metadata from that OfferDetail.
    """
    ...
    def validate_offer_detail_id(self, value):
        try:
            OfferDetail.objects.select_related("offer", "offer__user").get(pk=value)
